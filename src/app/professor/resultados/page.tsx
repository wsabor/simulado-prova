...
export default function ResultadosPage() {
  ...
  const exportCSV = () => {
    if (!data || data.resultados.length === 0) return;

    const header = 'Aluno,Email,Turma,Prova,Matérias,Questões,Nota,Status,Data';
    const rows = data.resultados.map((r) => {
      const status = (r.nota ?? 0) >= 6 ? 'Aprovado' : 'Reprovado';
      const dataStr = new Date(r.finalizada).toLocaleDateString('pt-BR');
      // Escapar campos que podem conter vírgula
      const materias = `"${r.provaMaterias.join(', ')}"`;
      const nome = r.alunoNome.includes(',') ? `"${r.alunoNome}"` : r.alunoNome;
      const prova = r.provaTitulo.includes(',') ? `"${r.provaTitulo}"` : r.provaTitulo;
      return `${nome},${r.alunoEmail},${r.alunoTurma},${prova},${materias},${r.totalQuestoes},${r.nota ?? '-'},${status},${dataStr}`;
    });
    ...
    link.download = `resultados_${timestamp}.csv`;
    link.click();
    URL.revokeObjectURL(url);
  };

  return (
    <ProfessorLayout titulo="Resultados" subtitulo="Desempenho dos alunos">
      {/* Filtros */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-bold text-gray-500 uppercase tracking-wider">Filtros</h2>
